...
class CoralBenchmarkTester {
    ...
    #printResultFunctions(
        ...
        for (const [name, subresults] of content) {
            ...
            console.log(
                `${prefix}+--> ${subresults.$fn.type.desugarAll.code} ${subresults.$fn.signature} (${subresults.result})${ptr}${excp}${globalsConverted}${externalCalls}`,
            );
        }
    }
        ...
        for (const subpath of Io.getPaths(this.#baseFolder + "/" + path)) {
            ...
            if (Io.isFolder(subpath)) {
                if (!isInner) {
                    ...
                    if (!subpath.getName().endsWith("3D_Image_Toolkit")) continue;
